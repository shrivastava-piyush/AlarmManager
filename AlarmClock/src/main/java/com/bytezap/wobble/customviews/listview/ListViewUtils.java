...
public class ListViewUtils {
    private static final String TAG = ListViewUtils.class.getName();

    private static class BackEaseListInterpolater implements Interpolator {
    ...
    private static class CircEaseListInterpolator implements Interpolator {
    ...
    public static void onTouchDown(CustomListView listView, MotionEvent ev) {
     ...
     * @return
     */
    public static boolean needListScale(CustomListView listView, int offset) {
    ...
    public static void resetScale(CustomListView listView) {
        listView.mInertia = 0;
        listView.setScaleY(1);
        listView.invalidate();
    }

    public static void onRenderTick(CustomListView listView, Canvas canvas) {
     ...
     * @param isTween
     */
    private static void setListScale(final CustomListView listView, Canvas canvas, int offset, boolean isTween) {
        if (offset == 0) {
        ...
        if (isTween) {
            ...
            scaleBackAnimator.setInterpolator(new BackEaseListInterpolater());
            ...
            if (listView.getScaleY() == 1) {
                ...
                scaleAnimator.setInterpolator(new CircEaseListInterpolator());
     ...
     * @return
     */
    private static boolean isScrollAtEdge(CustomListView listView, int offset) {
        ...
        if (childCount > 0) {
            ...
            //Log.v(TAG, "isScrollAtTop:" + isScrollAtTop + " offset:" + offset + " firstTop:" + firstTop);
