...
public class CropImage extends ImageView {
    ...
    public CropImage(Context context, AttributeSet attrs, int defStyle) {
        ...
        mHandleWidth = (int) (mDensity * HANDLE_SIZE_IN_DP);
    ...
    public Parcelable onSaveInstanceState() {
        ...
        ss.handleSize = this.mHandleWidth;
    ...
    public void onRestoreInstanceState(Parcelable state) {
        ...
        this.mHandleWidth = ss.handleSize;
    ...
    private void handleStyleable(Context context, AttributeSet attrs, int defStyle, float mDensity) {
        TypedArray ta = context.obtainStyledAttributes(attrs, R.styleable.CropImage, defStyle, 0);
        Drawable drawable;
        mCropMode = CropMode.RATIO_1_1;
        try {
            drawable = ta.getDrawable(R.styleable.CropImage_imgSrc);
            if (drawable != null) setImageDrawable(drawable);
            for (CropMode mode : CropMode.values()) {
                if (ta.getInt(R.styleable.CropImage_cropMode, 3) == mode.getId()) {
                    mCropMode = mode;
                    break;
                }
            }
            mBackgroundColor = ta.getColor(R.styleable.CropImage_backgroundColor, TRANSPARENT);
            super.setBackgroundColor(mBackgroundColor);
            mOverlayColor = ta.getColor(R.styleable.CropImage_overlayColor, TRANSLUCENT_BLACK);
            mFrameColor = ta.getColor(R.styleable.CropImage_frameColor, WHITE);
            mHandleColor = ta.getColor(R.styleable.CropImage_handleColor, WHITE);
            mGuideColor = ta.getColor(R.styleable.CropImage_guideColor, TRANSLUCENT_WHITE);
            for (ShowMode mode : ShowMode.values()) {
                if (ta.getInt(R.styleable.CropImage_guideShowMode, 1) == mode.getId()) {
                    mGuideShowMode = mode;
                    break;
                }
            }
            ...
            for (ShowMode mode : ShowMode.values()) {
                if (ta.getInt(R.styleable.CropImage_handleShowMode, 1) == mode.getId()) {
            ...
            mHandleWidth = ta.getDimensionPixelSize(R.styleable.CropImage_handlewidth, (int) (HANDLE_SIZE_IN_DP * mDensity));
            mTouchPadding = ta.getDimensionPixelSize(R.styleable.CropImage_touchPadding, 0);
            mMinFrameSize = ta.getDimensionPixelSize(R.styleable.CropImage_minFrameSize, (int) (MIN_FRAME_SIZE_IN_DP * mDensity));
            mFrameStrokeWeight = ta.getDimensionPixelSize(R.styleable.CropImage_frameStrokeWeight, (int) (FRAME_STROKE_WEIGHT_IN_DP * mDensity));
            mGuideStrokeWeight = ta.getDimensionPixelSize(R.styleable.CropImage_guideStrokeWeight, (int) (GUIDE_STROKE_WEIGHT_IN_DP * mDensity));
            mInitialFrameScale = constrain(ta.getFloat(R.styleable.CropImage_initialFrameScale, DEFAULT_INITIAL_FRAME_SCALE), 0.01f, 1.0f, DEFAULT_INITIAL_FRAME_SCALE);
        } catch (Exception e) {
            e.printStackTrace();
        } finally {
            ta.recycle();
    ...
    private void drawEditFrame(Canvas canvas) {
        ...
        if (mShowHandle) {
            ...
            mPaintFrame.setStrokeWidth(mFrameStrokeWeight - 1);
            float lateralOffset = -mTouchPadding;
            float startOffset = -mTouchPadding;
            canvas.drawLine(mFrameRect.left - lateralOffset, mFrameRect.top - startOffset, mFrameRect.left - lateralOffset, mFrameRect.top + mHandleWidth, mPaintFrame);
            canvas.drawLine(mFrameRect.left - startOffset, mFrameRect.top - lateralOffset, mFrameRect.left + mHandleWidth, mFrameRect.top - lateralOffset, mPaintFrame);
            canvas.drawLine(mFrameRect.left - lateralOffset, mFrameRect.bottom + startOffset, mFrameRect.left - lateralOffset, mFrameRect.bottom - mHandleWidth, mPaintFrame);
            canvas.drawLine(mFrameRect.left - startOffset, mFrameRect.bottom + lateralOffset, mFrameRect.left + mHandleWidth, mFrameRect.bottom + lateralOffset, mPaintFrame);
            canvas.drawLine(mFrameRect.right + lateralOffset, mFrameRect.top - startOffset, mFrameRect.right + lateralOffset, mFrameRect.top + mHandleWidth, mPaintFrame);
            canvas.drawLine(mFrameRect.right + startOffset, mFrameRect.top - lateralOffset, mFrameRect.right - mHandleWidth, mFrameRect.top - lateralOffset, mPaintFrame);
            canvas.drawLine(mFrameRect.right + lateralOffset, mFrameRect.bottom + startOffset, mFrameRect.right + lateralOffset, mFrameRect.bottom - mHandleWidth, mPaintFrame);
            canvas.drawLine(mFrameRect.right + startOffset, mFrameRect.bottom + lateralOffset, mFrameRect.right - mHandleWidth, mFrameRect.bottom + lateralOffset, mPaintFrame);
        }
    }
    ...
    private boolean isInsideCornerLeftTop(float x, float y) {
        ...
        float d = dx * dx + dy * dy;
        return sq(mHandleWidth + mTouchPadding) >= d;
    }
    ...
    private boolean isInsideRightBottom(float x, float y) {
        float dx = x - mFrameRect.right;
        float dy = y - mFrameRect.bottom;
        float d = dx * dx + dy * dy;
        return sq(mHandleWidth + mTouchPadding) >= d;
    ...
    public void setHandleSizeInDp(int handleDp) {
        mHandleWidth = (int) (handleDp * getDensity());
    }
